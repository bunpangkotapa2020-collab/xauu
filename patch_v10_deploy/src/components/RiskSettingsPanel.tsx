...
export function RiskSettingsPanel({ botState, onRefresh }: RiskSettingsPanelProps) {
  ...
  useEffect(() => {
    ...
    if (botState?.riskConfig) {
      if (botState.riskConfig.liveTradingEnabled !== undefined) {
          setLiveTradingEnabled(botState.riskConfig.liveTradingEnabled);
      }
      ...
      {isExpanded && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-5 border-t border-slate-800/80 pt-5">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="bg-slate-950/80 border border-slate-800 rounded-xl p-4 mt-2 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-sm font-bold text-white uppercase">LIVE TRADING</h3>
                </div>
                <p className="text-[10px] text-slate-400 mt-1">
                  {liveTradingEnabled 
                    ? '⚠️ គ្រោះថ្នាក់ (DANGER): Bot នឹងបាញ់ Order ទៅកាន់ទីផ្សារពិត។' 
              ...
              <button
                ...
                className={`relative px-5 py-2.5 rounded-xl font-bold text-xs uppercase tracking-wide transition-all ${
                  liveTradingEnabled 
              ...
              <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
                <div className="bg-slate-900 border border-red-500 rounded-2xl max-w-sm w-full p-6 shadow-2xl shadow-red-500/20">
                  ...
                  <p className="text-sm text-slate-300 mb-6 leading-relaxed">
                    តើអ្នកពិតជាចង់បើកដំណើរការ <strong>LIVE TRADING</strong> មែនទេ?
                    <br/><br/>
                    Bot នឹងចាប់ផ្តើមបាញ់ Order ដោយស្វ័យប្រវត្តិដោយប្រើប្រាស់លុយពិតរបស់អ្នក។ សូមត្រួតពិនិត្យ Lot Size ឲ្យបានច្បាស់លាស់។
                  </p>
                  <div className="flex justify-end gap-3">
                    ...
                    <button
                      ...
                      className="px-4 py-2 rounded-lg text-xs font-bold text-white bg-red-500 hover:bg-red-600 shadow-lg shadow-red-500/30"
                    >
                      យល់ព្រម (CONFIRM)
                    </button>
                  </div>
