...
import { ICTRealMarketAdapter } from './src/ICT_RealMarketAdapter.js';
import { RealNewsProvider } from './src/ICT_NewsAdapter.js';

const USE_ICT_EA = true;

const ictNewsProvider = new RealNewsProvider();
const ictEaConfig: EAConfig = {
    symbol: 'XAUUSD',
    isRealAccount: true,
    isUSCAccount: true,
    lotSize: 0.01,
    ...
    cooldownMinutes: 15,
    maxOpenTrades: 4,
    tradingSessionStart: "08:00",
    tradingSessionEnd: "22:00",
    newsFilterEnabled: true,
    minutesBeforeNewsBlock: 30,
    minutesAfterNewsBlock: 30,
    obLookbackCandles: 20,
    obMinDisplacementBodyRatio: 0.5,
...
class RealMetaApiExecution {
    logDiagnosticContext(symbol, direction, lot, sl, tp, magic) {
        ...
        console.log(`Live Trading Enabled: ${ictEaEngine.LIVE_TRADING_ENABLED ? 'YES' : 'NO'}`);
...
const ictEaEngine = new IctXauusdEA(ictEaConfig, ictNewsProvider, new RealMetaApiExecution());
...
ictEaEngine.onSetupConfirmed = async (data) => {
    try {
        await sendIctSetupConfirmedAlert(data);
    } catch (err) {
        console.error('[Telegram] Error sending ICT Setup Confirmed alert:', err);
    }
};
...
ictEaEngine.onActualEntryTriggered = async (data) => {
    try {
        const t0 = new Date().toISOString();
        console.log(`[${t0}] TELEGRAM ALERT TRIGGERED: Actual Entry Reached`);
        // await sendActualEntryAlert(data); // Disabled per requested flow
        const t1 = new Date().toISOString();
        console.log(`[${t1}] TELEGRAM ALERT SENT`);
    } catch (err) {
        console.error('[Telegram] Error sending Actual Entry alert:', err);
    }
};
...
ictEaEngine.onExecutionFailed = async (data) => {
    try {
        const msg = `🔴 EXECUTION FAILED\n` +
                    `Setup ID: ${data.setupId}\n` +
                    `Reason: ${data.reason}\n` +
                    `Action: Order was rejected by MetaAPI/MT5. EA will not retry this setup to prevent spam.`;
        await sendTelegramRaw(msg, `EXEC_FAILED_${data.setupId}`, 0);
    } catch (err) {
        console.error('[Telegram] Error sending Execution Failed alert:', err);
    }
};
...
const systemComponentStates: Record<string, { isHealthy: boolean; lastError: string }> = {
    ...
    'NEW ICT EA': { isHealthy: true, lastError: '' }
};
    ...
    if (type === 'PROBLEM') {
        ...
        if (component === 'MT5 Connection') {
            ...
            const componentKhmer = component === 'Market Data' ? 'ទិន្នន័យទីផ្សារ (Market Data)' :
                                   ...
                                   component === 'NEW ICT EA' ? 'ប្រព័ន្ធ NEW ICT EA' : component;
...
function syncConfigToIctEa(state) {
  if (typeof ictEaEngine !== 'undefined' && ictEaEngine.config) {
    if (state.riskConfig?.lotSize !== undefined) ictEaEngine.config.lotSize = state.riskConfig.lotSize;
    if (state.riskConfig?.stopLossPips !== undefined) ictEaEngine.config.stopLossDistance = state.riskConfig.stopLossPips;
    if (state.riskConfig?.takeProfitPips !== undefined) ictEaEngine.config.takeProfitDistance = state.riskConfig.takeProfitPips;
    if (state.riskConfig?.maxDailyLossAmount !== undefined) ictEaEngine.config.dailyLossLimit = state.riskConfig.maxDailyLossAmount;
    if (state.riskConfig?.maxConsecutiveLosses !== undefined) ictEaEngine.config.maxConsecutiveSL = state.riskConfig.maxConsecutiveLosses;
    if (state.riskConfig?.cooldownMinutes !== undefined) ictEaEngine.config.cooldownMinutes = state.riskConfig.cooldownMinutes;
    if (state.riskConfig?.maxOpenTrades !== undefined) ictEaEngine.config.maxOpenTrades = state.riskConfig.maxOpenTrades;
  }
}
...
function resetEASetup(reason: string) {
  console.log(`[ICT EA] RESET SETUP - REASON = ${reason}`);
...
async function manageTrailingSL() {
    ...
    const spreadInGold = (botState.askPrice && botState.bidPrice && botState.askPrice > botState.bidPrice)
        ? (botState.askPrice - botState.bidPrice)
        : ((botState.spreadPoints || 20) / 100);
    ...
    for (let trade of botState.openTrades) {
        ...
        const potentialNewSl = ictEaEngine.evaluateTrailingSL(
            ...
            trade.originalTp || trade.tp || 0,
            spreadInGold
        );
        
        if (potentialNewSl !== null && potentialNewSl !== trade.sl && potentialNewSl !== 0) {
            const oldSlVal = trade.sl;
            const isProfitLock = ictEaEngine.isProfitLockActive(trade.id);
            trade.sl = potentialNewSl;
            trade.trailingSlValue = potentialNewSl;
            botState.statusMessageKhmer = isProfitLock 
                ? `🔒 [PROFIT LOCK] SL TRAILING -> ${potentialNewSl}`
                : `🔒 PROFIT LOCKING -> SL TRAILING -> ${potentialNewSl}`;
            ...
            if (accountId && token && baseUrl && botState.account.serverConnected) {
                 ...
                 // If profit lock is active and broker has hard TP, remove/widen hard TP so trade can continue running
                 if (isProfitLock) {
                     if (!trade.originalTp && trade.tp && trade.tp > 0) {
                         trade.originalTp = trade.tp;
                     }
  ...
  try {
    ...
    if (currentAsk <= 0 || currentBid <= 0 || priceAgeMs > 60000) {
        ...
        if (s && s.stage !== 'INVALIDATED') {
            ...
            botState.signalDetails = undefined;
            if (isRunning) {
              botState.statusMessageKhmer = `🔴 [ICT EA SAFE-MODE] ទីផ្សារគ្មានទិន្នន័យ Live Price ពិតប្រាកដ — ផ្អាកការវិភាគ និងបិទការបើក Trade ជាបណ្តោះអាសន្ន`;
            }
        }
        return;
    }
    ...
    if (isRunning) {
      botState.statusMessageKhmer = `⏳ [ICT EA] Analyzing Real Market (${symbolToTrade})...`;
    }
    
    // Sequential candle fetching across H4, M15, M1 to avoid 429 rate limit errors
    ...
    if (h4Candles.length > 0 && m15Candles.length > 0 && m1Candles.length > 0) {
        ...
        ictEaEngine.config.symbol = symbolToTrade;
        ...
        if (ictEaEngine.state.currentSetup && ictEaEngine.state.currentSetup.stage !== 'INVALIDATED') {
           const s = ictEaEngine.state.currentSetup;
           if (isRunning) {
             botState.statusMessageKhmer = `🔍 [ICT EA SAFE-MODE] ${s.bias} READY `;
           }
           const exactEntryVal = s.lockedEntryPrice || (s.bias === 'BULLISH' ? s.obHigh : s.obLow);
           const exactSlVal = s.lockedSlTarget;
            const exactTpVal = s.lockedTpTarget;
...
async function syncDailyRealizedPnL() {
    ...
    try {
        ...
        if (error.name === 'TimeoutError') {
            ...
            // No backoff applied to preserve Risk Guard
            lastPnLSyncTime = Date.now(); 
        } else {
            console.error('[ICT EA] P/L Sync Error:', error.message || error);
        }
        }
        // Fallback gracefully so bot does not loop-fail or block trading
        botState.isDailyPnLSynced = false; // FAIL-SAFE: Block entry on sync error
    ...
    if (accountId && token && baseUrl && (baseUrl.includes('agiliumtrade.ai') || baseUrl.includes('metaapi.cloud'))) {
        try {
            ...
            try {
                ...
                if (!quoteRes.ok && quoteRes.status !== 429) console.log(`[QUOTE ERROR] ${quoteRes.status} ${quoteRes.statusText} URL: ${workingBaseUrl}/users/current/accounts/${accountId}/symbols/${primarySymbol}/current-price`);
                let quote = null;
                if (quoteRes.ok) {
                ...
                if (quoteRes.ok && quote) {
                    if (quote && (quote.bid || quote.ask || quote.price)) {
                        ...
                        if (bid > 0) {
                            ...
                            const tickTimeMs = quote.time ? new Date(quote.time).getTime() : Date.now();
                            botState.lastTickTime = tickTimeMs;
                            ...
                            if (botState.status !== 'running') {
                                ictEaEngine.LIVE_TRADING_ENABLED = false;
                                ictEaEngine.liveTradingBlockReason = "Bot is STOPPED";
                            } else if (!botState.account.isConnected || !botState.account.serverConnected) {
                                ictEaEngine.LIVE_TRADING_ENABLED = false;
                                ictEaEngine.liveTradingBlockReason = "MT5 Disconnected";
                            } else if (!isMarketOpen) {
                                ictEaEngine.LIVE_TRADING_ENABLED = false;
                                ictEaEngine.liveTradingBlockReason = "Market Closed";
                            } else if (!botState.isDailyPnLSynced) {
                                ictEaEngine.LIVE_TRADING_ENABLED = false;
                                ictEaEngine.liveTradingBlockReason = "Daily P/L Syncing";
                            } else if (botState.dailyLossLimitHit) {
                                ictEaEngine.LIVE_TRADING_ENABLED = false;
                                ictEaEngine.liveTradingBlockReason = "Daily Loss Limit Hit";
                            } else if (botState.marketSpeed === 'EXTREME') {
                                ictEaEngine.LIVE_TRADING_ENABLED = false;
                                ictEaEngine.liveTradingBlockReason = "Extreme Volatility";
                            } else {
                                ictEaEngine.LIVE_TRADING_ENABLED = true;
                                ictEaEngine.liveTradingBlockReason = "";
                            }
                            // ----------------------------------------------------

                            // Forward live tick to ICT Market Adapter
                            if (ictMarketAdapter && typeof (ictMarketAdapter as any).onTick === 'function') {
                                try {
                                    (ictMarketAdapter as any).onTick(primarySymbol, bid, ask, Date.now());
                                } catch (e) {
                                    console.error('[ICT EA] Error forwarding tick:', e);
                                }
                            }
                ...
                if (botState.openTrades && botState.openTrades.length > 0) {
                    ...
                    botState.openTrades.forEach((oldTrade: any) => {
                        if (!newTradeIds.includes(oldTrade.id)) {
                            tradeClosed = true;
                            if (ictEaEngine?.tradeProfitLockState) {
                                ictEaEngine.tradeProfitLockState.delete(String(oldTrade.id));
                            }
                            if (oldTrade.floatingProfit < 0) {
                                if (typeof eaState !== 'undefined') {
                                   eaState.consecutiveLosses += 1;
                                   console.log(`[ICT EA] Trade ${oldTrade.id} closed in LOSS. Consecutive Losses: ${eaState.consecutiveLosses}`);
                                ...
                                if (typeof eaState !== 'undefined') {
                                   ...
                                   console.log(`[ICT EA] Trade ${oldTrade.id} closed in PROFIT. Consecutive Losses reset to 0.`);
                                }
                                sendTakeProfitAlert({
                                    type: (oldTrade.side === 'BUY' || oldTrade.side === 'SELL') ? oldTrade.side : 'BUY',
                                    lot: oldTrade.lot || botState.riskConfig?.lotSize || 0.01,
...
const app = express();
  ...
  app.get('/api/bot/state', (req, res) => {
    ...
    if (ictEaEngine && typeof ictEaEngine.getTelemetry === 'function') {
        botState.ictAnalysis = ictEaEngine.getTelemetry(botState);
    }
    ...
    const startConfirmation = {
      ...
      currentWaitingReason: botState.ictAnalysis?.waitingReason || '⏳ WAITING FOR M15 LIQUIDITY SWEEP',
      isEntriesBlocked,
    ...
    res.json({
      ...
      liveEaConfig: {
        ...
        currentSetupStage: ictEaEngine.state.currentSetup?.stage || 'SEARCHING',
        currentSetupId: ictEaEngine.state.currentSetup?.id || null,
        entryZone: ictEaEngine.state.currentSetup ? `[${ictEaEngine.state.currentSetup.obLow?.toFixed(3)} - ${ictEaEngine.state.currentSetup.obHigh?.toFixed(3)}]` : null,
        actualEntryPrice: ictEaEngine.state.currentSetup?.lockedEntryPrice || (ictEaEngine.state.currentSetup ? (ictEaEngine.state.currentSetup.bias === 'BULLISH' ? ictEaEngine.state.currentSetup.obHigh : ictEaEngine.state.currentSetup.obLow) : null),
        triggerStatus: ictEaEngine.state.currentSetup?.stage === 'TRIGGERED' ? 'ENTRY TRIGGERED' : (ictEaEngine.state.currentSetup ? 'WAITING' : 'SEARCHING')
      }
    });
  });
...
app.post('/api/bot/action', async (req, res) => {
    ...
    if (action === 'select_asset') {
      ...
      if (!ictEaEngine.state.currentSetup || ictEaEngine.state.currentSetup.stage === 'INVALIDATED') { botState.signals = { gold: 'WAIT' };
      botState.signalDetails = undefined; }
      botState.tickHistory = [];
      ...
      // Reset any active ICT EA setups
      if (ictEaEngine && ictEaEngine.state) {
        ictEaEngine.state.currentSetup = null;
      }
  ...
  app.post('/api/bot/update-risk-config', requireAdminAuth, (req, res) => {
    ...
    if (riskConfig) {
      ...
      if (typeof ictEaEngine !== 'undefined' && ictEaEngine.config) {
        if (botState.riskConfig.lotSize !== undefined) ictEaEngine.config.lotSize = botState.riskConfig.lotSize;
        if (botState.riskConfig.stopLossPips !== undefined) ictEaEngine.config.stopLossDistance = botState.riskConfig.stopLossPips;
        if (botState.riskConfig.takeProfitPips !== undefined) ictEaEngine.config.takeProfitDistance = botState.riskConfig.takeProfitPips;
        if (botState.riskConfig.maxDailyLossAmount !== undefined) ictEaEngine.config.dailyLossLimit = botState.riskConfig.maxDailyLossAmount;
        if (botState.riskConfig.maxConsecutiveLosses !== undefined) ictEaEngine.config.maxConsecutiveSL = botState.riskConfig.maxConsecutiveLosses;
        if (botState.riskConfig.cooldownMinutes !== undefined) ictEaEngine.config.cooldownMinutes = botState.riskConfig.cooldownMinutes;
        if (botState.riskConfig.maxOpenTrades !== undefined) ictEaEngine.config.maxOpenTrades = botState.riskConfig.maxOpenTrades;
      }
      
      // CRITICAL FIX: Sync new config into the DaRa M1 Engine instance
      if (typeof daraEngine !== 'undefined') {
          if (riskConfig.liveTradingEnabled !== undefined) {
              botState.riskConfig.liveTradingEnabled = Boolean(riskConfig.liveTradingEnabled);
          }
