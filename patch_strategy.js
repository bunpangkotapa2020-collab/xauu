...
code = code.replace(
/let displacementFound = false;\s*let displacementIdx = -1;\s*for \(let i = sweepIdx; i < candles\.length; i\+\) \{/g,
`let displacementFound = false;
      let displacementIdx = -1;
...
code = code.replace(
/let mssConfirmed = false;\s*let mssCandle: DaRaCandle \| null = null;\s*for \(let i = displacementIdx; i < candles\.length; i\+\) \{/g,
`let mssConfirmed = false;
      let mssCandle: DaRaCandle | null = null;
