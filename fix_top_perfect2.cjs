...
if (importStart > -1 && dotenvIndex > -1) {
    ...
    // find the closing brace of the interface by searching for "const botState" or "const DEFAULT_BOT_CONFIG"
    let interfaceEnd = finalCode.indexOf('const DEFAULT_BOT_CONFIG', interfaceStart);
    if (interfaceEnd === -1) interfaceEnd = finalCode.indexOf('const botState', interfaceStart);
