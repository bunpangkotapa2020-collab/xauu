...
// Inside executeTrade, change the BUY logic
const buyLogicOld = /if \(s\.bias === 'BULLISH'\) \{[\s\S]*?this\.state\.executedSetupIds\.add\(s\.id\);\n\s*\}/;

const buyLogicNew = `if (s.bias === 'BULLISH') {
            entry = data.ask;
            sl = entry - 10.000;
            tp = entry + 10.000;
...
code = code.replace(buyLogicOld, buyLogicNew);

// Inside executeTrade, change the SELL logic
const sellLogicOld = /else if \(s\.bias === 'BEARISH'\) \{[\s\S]*?this\.state\.executedSetupIds\.add\(s\.id\);\n\s*\}/;

const sellLogicNew = `else if (s.bias === 'BEARISH') {
            entry = data.bid;
            sl = entry + 10.000;
            tp = entry - 10.000;
...
code = code.replace(sellLogicOld, sellLogicNew);
