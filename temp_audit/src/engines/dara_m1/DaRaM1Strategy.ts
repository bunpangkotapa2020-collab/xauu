...
export class DaRaM1Strategy {
  ...
  public scanForSetup(
    ...
    if (buySetup) {
      buySetup.executionDirection = 'SELL';
      buySetup.masterEntryPrice = buySetup.lockedEntryPrice;
      // Original BUY expected price to go UP. Executed SELL expects price to go DOWN.
      // Recalculate SL (above) and TP (below) using SELL logic from the immutable Master/Locked Entry:
    ...
    if (sellSetup) {
      sellSetup.executionDirection = 'BUY';
      sellSetup.masterEntryPrice = sellSetup.lockedEntryPrice;
      // Original SELL expected price to go DOWN. Executed BUY expects price to go UP.
      // Recalculate SL (below) and TP (above) using BUY logic from the immutable Master/Locked Entry:
