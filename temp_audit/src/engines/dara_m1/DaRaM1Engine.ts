    ...
    if ((state === 'WAIT_FOR_LOCKED_ENTRY' || state === 'TRADE_ACTIVE') && currentSetup) {
      ...
      if (isReached) {
        ...
        if (this.userSettings.liveTradingEnabled !== true) {
          ...
          console.log(`[DaRa M1 EA v1.0] ℹ️ Level ${positionNumber} Target reached (${currentPrice} - target: ${nextLevel.targetPrice}). Safe Monitor Mode active (Live Trading is OFF) — Paper simulation position recorded. (Original Signal=${currentSetup.direction}, Executing=${execDir})`);
        ...
        const execDir = currentSetup.executionDirection || currentSetup.direction;
        console.log(`[DaRa M1 EA v1.0] ⚡ Level ${positionNumber} Target reached (${currentPrice} - target: ${nextLevel.targetPrice}). Executing Position #${positionNumber} ${execDir} (Original=${currentSetup.direction})...`);
        if (this.telegram) {
