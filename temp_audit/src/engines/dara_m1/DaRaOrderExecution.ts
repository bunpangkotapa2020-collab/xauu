...
export class DaRaOrderExecution {
  ...
  public async executeOrder(
    ...
    console.log(`[DaRa EXECUTION AUDIT] 📝 SetupID=${setup.id} | OrigDir=${setup.direction} | ExecDir=${execDir} | MasterEntry=${masterEntry} | EntryDist=${settings.entryDistance ?? 1.0} | Level=${levelIndex + 1} | TargetPrice=${setup.entryLevels?.[levelIndex]?.targetPrice} | ConfigPositionsPerSetup=${maxAllowedPositions} | CurrentPositionsOpened=${setup.positionsOpened || 0} | SL=${sl} | TP=${tp}`);

    this.isExecutionInProgress = true;

    try {
      console.log(`[DaRa M1 EA v1.0] 🛡️ PRE-FLIGHT VERIFIED (SINGLE SOURCE OF TRUTH 100%): Direction=${execDir} (Original Signal=${setup.direction}), Lot=${lot} (user), SL=${sl} (dist: ${slDistance}), TP=${tp} (dist: ${tpDistance}), Entry=${openPrice}`);
