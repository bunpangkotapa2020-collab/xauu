...
for (let i = 0; i < lines.length; i++) {
    if (lines[i].includes('private validateRetracement(data: ICTMarketData) {')) {
        start = i;
    }
    if (start !== -1 && i > start && lines[i].includes('public LIVE_TRADING_ENABLED = false;')) {
        end = i;
        break;
    }
}
...
if (start !== -1 && end !== -1) {
    console.log(`Replacing lines ${start} to ${end}`);
